...
This copies the pipeline (the scripts and `requirements.txt`) into the target's `.tools/`, refreshing them on every run so a re-run pushes the latest version. The content files (`config.toml`, `template.html`, `style.css`, `robots.txt`) are written only if absent, so your edits are never clobbered; pass `--override` (all of them) or name specific files (e.g. `--override template.html`) to refresh those to the shipped defaults. It also ensures `.gitignore` excludes `.public/`.
